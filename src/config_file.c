...
gboolean load_config(const gchar *filename, RaucConfig **config, GError **error)
{
	 ...
	 * - status per slot and no data directory
	 *   (by default or explicitly with statusfile=per-slot)
	 * - central status and no data directory
	 *   (statusfile=/data/central.raucs)
	 * - central status and data directory
	 *   (statusfile=/data/central.raucs and data-directory=/data/rauc)
	 * - central status in data directory
	 *   (data-directory=/data/rauc, implicit statusfile=/data/rauc/central.rauc)
	 */
	...
	for (gsize i = 0; i < group_count; i++) {
		...
		if (g_str_equal(groupsplit[0], RAUC_SLOT_PREFIX)) {
			 ...
			 * subdirectory for pre-slot data. */
			if (c->data_directory)
