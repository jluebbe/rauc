...
typedef struct {
	...
	RaucManifest *manifest; /* from the last successful attempt */
	gboolean update_available;
	gchar *summary;
	gboolean update_failed;
} RPollSource;
...
static gboolean poll_install(RPollSource *poll_source, GError **error)
{
	...
	args->notify = service_install_notify; /* FIXME? */
	args->cleanup = service_install_cleanup; /* FIXME? */
...
static gboolean poll_step(RPollSource *poll_source, GError **error)
{
	...
	/* TODO only retry if manifest hash changes? */
	if (!poll_source->update_failed) {
		if (!poll_install(poll_source, &ierror)) {
			g_propagate_prefixed_error(error, ierror, "starting installation failed: ");
			poll_source->update_failed = TRUE;
			return FALSE;
		} else {
			poll_source->update_failed = FALSE;
		}
	}
...
static void poll_update_status(RPollSource *poll_source)
{
	...
	g_variant_builder_add(&builder, "{sv}", "update-failed", g_variant_new_boolean(poll_source->update_failed));
