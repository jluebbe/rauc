...
{
	GError *ierror = NULL;
	g_autoptr(GByteArray) hashes = g_byte_array_set_size(g_byte_array_new(), ((guint)count)*32);
	...
	for (guint32 i = 0; i < count; i++) {
		...
		hash_chunk(chunk);
		memcpy(&hashes->data[i*32], chunk->hash, 32);
	}
...
static gint lookup_compare_sort(gconstpointer a, gconstpointer b, gpointer hashes)
{
	...
	const guint8 *a_hash = _hashes + *_a * 32;
	const guint8 *b_hash = _hashes + *_b * 32;
	int res;

	res = memcmp(a_hash, b_hash, 32);
	if (res)
		return res;
...
static guint32 *build_lookup(GBytes *hashes)
{
	...
	count = g_bytes_get_size(hashes) / 32;
...
static guint32 get_chunk_count(int data_fd, GError **error)
{
	...
	/* Verify that the data file as a reasonable size. */
	if (size == 0) {
		g_set_error(error,
				R_HASH_INDEX_ERROR,
				R_HASH_INDEX_ERROR_SIZE,
...
RaucHashIndex *r_hash_index_reuse(const gchar *label, const RaucHashIndex *idx, int new_data_fd, GError **error)
{
	...
	new_idx->hashes = g_bytes_new_from_bytes(idx->hashes, 0, new_idx->count * 32);
...
gboolean r_hash_index_get_chunk(const RaucHashIndex *idx, const guint8 *hash, RaucHashIndexChunk *chunk, GError **error)
{
	GError *ierror = NULL;
	gboolean ret = FALSE;
	const guint8(*hashes)[32];
	guint32 left, middle, right;
	gboolean found = FALSE;
	off_t offset;

	G_STATIC_ASSERT(sizeof(hashes[0]) == 32);
	...
	while (left <= right) {
		...
		cmp = memcmp(hashes[idx->lookup[middle]], hash, 32);
		if (cmp == 0) {
			found = TRUE;
			break;
		} else if (cmp < 0) {
	...
	for (guint32 i = middle; i > 0; i--) {
		...
		if (memcmp(hashes[next], hash, 32) != 0) {
			middle = i;
			break;
		}
	}
	...
	for (guint32 i = middle; i < idx->count; i++) {
		...
		if (memcmp(hashes[curr], hash, 32) != 0)
			break;
	...
	if (!idx->skip_hash_check) {
		hash_chunk(chunk);
		if (memcmp(chunk->hash, hash, 32) != 0) {
			g_set_error(error,
					R_HASH_INDEX_ERROR,
