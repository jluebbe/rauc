...
{
	g_message("%s count=%"G_GUINT64_FORMAT, prefix, stats->count);
	if (!stats->count)
		return;
	g_message("%s sum=%.3f min=%.3f max=%.3f avg=%.3f", prefix,
			stats->sum, stats->min, stats->max, r_stats_get_avg(stats));
	g_message("%s recent avg=%.3f", prefix, r_stats_get_recent_avg(stats));
}
...
static size_t header_cb(char *buffer, size_t size, size_t nitems, void *userdata)
{
	...
	/* TODO read data and last modified? */
	...
	if (g_str_equal(h_pair[0], "content-range")) {
		...
		g_assert(g_strv_length(h_elements) == 2);

		h_range = g_strsplit(h_elements[1], "/", 2);
		g_assert(g_strv_length(h_range) == 2);

		g_assert_cmpstr(h_range[0], ==, "0-3");
		g_assert(strlen(h_range[1]) > 0);
		g_assert_cmpstr(h_range[1], !=, "*");
	...
	if (g_str_equal(h_pair[0], "last-modified")) {
		time_t date = curl_getdate(h_pair[1], NULL);
		if (date >= 0) {
...
gboolean start_nbd_server(RaucNBDServer *nbd_srv, GError **error)
{
	...
	if (socketpair(AF_UNIX, SOCK_STREAM, 0, sockets) < 0)
		g_error("failed to create unix socket pair");
