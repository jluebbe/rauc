...
static void check_manifest_contents(const RaucManifest *rm)
{
	...
	g_assert_cmpint(g_strv_length(image->converted), ==, 2);
	g_assert_cmpstr(image->converted[0], ==, "invalid-converted");
	g_assert_cmpstr(image->converted[1], ==, "another-invalid-converted");
...
static void test_save_load_manifest(void)
{
	...
	new_image->converted = g_strsplit("invalid-converted;another-invalid-converted", ";", 0);
	rm->images = g_list_append(rm->images, new_image);
