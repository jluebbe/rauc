...
This functionality is designed to automate the update process and ensure that
the system remains up-to-date with minimal manual intervention.

The polling functionality is designed such that it can be used and extended for
different scenarios:
...
be configured in via ``install-if`` in the ``[poll]`` section.
In simple cases, RAUC can just install any valid update and trigger a reboot.
For more complex cases, RAUC can still handle the polling, while a separate
application uses the collected information to trigger an installation.

Currently, RAUC considers an update valid if the ``version`` in the ``[update]``
manifest section differs from the ``RAUC_SYSTEM_VERSION`` as reported by the
``system-info`` handler.

FIXME: add semver comparison?
...
Confirmation and Manifest Hashes
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
increase the polling interval to resolve an overload scenario.
...
The :ref:`Poller interface <gdbus-interface-de-pengutronix-rauc-Poller>` can be
used to trigger a poll and also exposes the results.
